...
$.DateTimePicker = $.DateTimePicker || {
    ...
    // Temporary Variables For Calculation Specific to DateTimePicker Instance
    dataObject: {
...
$.cf = {
    ...
    _compare: function (sString1, sString2) {
        ...
        if (bString1 && bString2) {
            if (sString1.toLowerCase() === sString2.toLowerCase()) return true;
            else return false;
        } else return false;
    },
};
...
(function (factory) {
    ...
    $.fn.DateTimePicker = function (options) {
        ...
        if (typeof options === 'string') {
            ...
            return this.each(function () {
                ...
                if (!$.data(this, 'plugin_DateTimePicker')) $.data(this, 'plugin_DateTimePicker', new DateTimePicker(this, options));
            });
        }
    };
    ...
    DateTimePicker.prototype = {
        ...
        init: function () {
            ...
            if (oDTP.settings.init) oDTP.settings.init.call(oDTP);
        ...
        _matchFormat: function (sMode, sFormat) {
            ...
            for (iTempIndex = 0; iTempIndex < oArrInput.length; iTempIndex++) {
                oDTP.oData.bArrMatchFormat.push($.cf._compare(sFormat, oArrInput[iTempIndex]));
            }
        },
        ...
        _setMatchFormat: function (iArgsLength, sMode, sFormat) {
            ...
            if (iArgsLength > 0) oDTP._matchFormat(sMode, sFormat);
        },
        ...
        _addEventHandlersForInput: function () {
            ...
            if (!oDTP.settings.isInline) {
                ...
                $(oDTP.settings.parentElement)
                    .find("input[type='date'], input[type='time'], input[type='datetime']")
                    .each(function () {
                ...
                $(oDTP.settings.parentElement).off('focus', sel, oDTP._inputFieldFocus).on('focus', sel, { obj: oDTP }, oDTP._inputFieldFocus);

                $(oDTP.settings.parentElement).off('click', sel, oDTP._inputFieldClick).on('click', sel, { obj: oDTP }, oDTP._inputFieldClick);
            }

            if (oDTP.settings.addEventHandlers) oDTP.settings.addEventHandlers.call(oDTP);
        },
        ...
        getDateObjectForInputField: function (oInputField) {
            ...
            if ($.cf._isValid(oInputField)) {
                ...
                if (!$.cf._isValid(sMode)) sMode = oDTP.settings.mode;
                if (!oDTP.settings.formatDateTimeString) {
                    sFormat = $(oInputField).data('format');
                    if (!$.cf._isValid(sFormat)) {
                        if ($.cf._compare(sMode, 'date')) sFormat = oDTP.settings.dateFormat;
                        else if ($.cf._compare(sMode, 'time')) sFormat = oDTP.settings.timeFormat;
                        else if ($.cf._compare(sMode, 'datetime')) sFormat = oDTP.settings.dateTimeFormat;
                    }

                    oDTP._matchFormat(sMode, sFormat);

                    if ($.cf._compare(sMode, 'date')) dInput = oDTP._parseDate(sDateTime);
                    else if ($.cf._compare(sMode, 'time')) dInput = oDTP._parseTime(sDateTime);
                    else if ($.cf._compare(sMode, 'datetime')) dInput = oDTP._parseDateTime(sDateTime);
                } else {
        ...
        setDateTimeStringInInputField: function (oInputField, dInput) {
            ...
            if ($.cf._isValid(oInputField)) {
                oArrElements = [];
                if (typeof oInputField === 'string') oArrElements.push(oInputField);
                else if (typeof oInputField === 'object') oArrElements = oInputField;
            } else {
        ...
        showDateTimePicker: function (oElement) {
            ...
            if (oDTP.oData.oInputElement !== null) {
                if (!oDTP.settings.isInline) oDTP._hidePicker(0, oElement);
            } else oDTP._showPicker(oElement);
        },
        ...
        _setButtonAction: function (bFromTab) {
            ...
            if (oDTP.oData.oInputElement !== null) {
                ...
                if (bFromTab) {
                    if (oDTP.settings.buttonClicked) oDTP.settings.buttonClicked.call(oDTP, 'TAB', oDTP.oData.oInputElement);
                    if (!oDTP.settings.isInline) oDTP._hidePicker(0);
                } else {
                    if (!oDTP.settings.isInline) oDTP._hidePicker('');
                }
            }
        },
        ...
        _setOutput: function (sMode, sFormat, bIs12Hour, dCurrentDate, oElement) {
            ...
            if (oDTP.settings.formatDateTimeString) {
                ...
                if (oDTP.oData.bDateMode) {
                    ...
                    if (sDateStr !== '' && sTimeStr !== '') sOutput = sDateStr + oDTP.settings.dateTimeSeparator + sTimeStr;
                }
        ...
        _showPicker: function (oElement) {
            ...
            if (oDTP.oData.oInputElement === null) {
                ...
                if (sView !== '') {
                    if ($.cf._compare(sView, 'Popup')) oDTP.setIsPopup(true);
                    else oDTP.setIsPopup(false);
                }
                ...
                if (oDTP.settings.beforeShow) oDTP.settings.beforeShow.call(oDTP, oElement);
                ...
                var sMin, sMax, sTempDate, dTempDate, sTempTime, dTempTime, sTempDateTime, dTempDateTime;
                ...
                if (oDTP.oData.bDateMode) {
                    ...
                    if ($.cf._isValid(sMin)) oDTP.oData.dMinValue = oDTP._parseDate(sMin);
                    if ($.cf._isValid(sMax)) oDTP.oData.dMaxValue = oDTP._parseDate(sMax);
                    ...
                    if (sStartEnd !== '' && ($.cf._compare(sStartEnd, 'start') || $.cf._compare(sStartEnd, 'end')) && sStartEndElem !== '') {
                        if ($(sStartEndElem).length >= 1) {
                            ...
                            if (sTempDate !== '') {
                                if (oDTP.settings.parseDateTimeString) dTempDate = oDTP.settings.parseDateTimeString.call(oDTP, sTempDate, sMode, sFormat, $(sStartEndElem));
                                else dTempDate = oDTP._parseDate(sTempDate);

                                if ($.cf._compare(sStartEnd, 'start')) {
                                    if ($.cf._isValid(sMax)) {
                                        if (oDTP._compareDates(dTempDate, oDTP.oData.dMaxValue) < 0) oDTP.oData.dMaxValue = new Date(dTempDate);
                                    } else oDTP.oData.dMaxValue = new Date(dTempDate);
                                } else if ($.cf._compare(sStartEnd, 'end')) {
                                    if ($.cf._isValid(sMin)) {
                                        if (oDTP._compareDates(dTempDate, oDTP.oData.dMinValue) > 0) oDTP.oData.dMinValue = new Date(dTempDate);
                    ...
                    if (oDTP.settings.parseDateTimeString) oDTP.oData.dCurrentDate = oDTP.settings.parseDateTimeString.call(oDTP, sCurrent, sMode, sFormat, $(oElement));
                    else oDTP.oData.dCurrentDate = oDTP._parseDate(sCurrent);
                    ...
                    if ($.cf._isValid(sMin)) {
                        ...
                        if (!$.cf._isValid(sMax)) {
                            if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[0]) sMax = '11:59:59 PM';
                            else if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[1]) sMax = '23:59:59';
                            else if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[2]) sMax = '11:59 PM';
                            else if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[3]) sMax = '23:59';
                    ...
                    if ($.cf._isValid(sMax)) {
                        ...
                        if (!$.cf._isValid(sMin)) {
                            if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[0]) sMin = '12:00:00 AM';
                            else if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[1]) sMin = '00:00:00';
                            else if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[2]) sMin = '12:00 AM';
                            else if (oDTP.oData.sTimeFormat === oDTP.oData.sArrInputTimeFormats[3]) sMin = '00:00';
                    ...
                    if (sStartEnd !== '' && ($.cf._compare(sStartEnd, 'start') || $.cf._compare(sStartEnd, 'end')) && sStartEndElem !== '') {
                        if ($(sStartEndElem).length >= 1) {
                            ...
                            if (sTempTime !== '') {
                                if (oDTP.settings.parseDateTimeString) dTempDate = oDTP.settings.parseDateTimeString.call(oDTP, sTempTime, sMode, sFormat, $(sStartEndElem));
                                else dTempTime = oDTP._parseTime(sTempTime);

                                if ($.cf._compare(sStartEnd, 'start')) {
                                    dTempTime.setMinutes(dTempTime.getMinutes() - 1);
                                    if ($.cf._isValid(sMax)) {
                                        if (oDTP._compareTime(dTempTime, oDTP.oData.dMaxValue) === 2) oDTP.oData.dMaxValue = new Date(dTempTime);
                                    } else oDTP.oData.dMaxValue = new Date(dTempTime);
                                } else if ($.cf._compare(sStartEnd, 'end')) {
                                    dTempTime.setMinutes(dTempTime.getMinutes() + 1);
                                    if ($.cf._isValid(sMin)) {
                                        if (oDTP._compareTime(dTempTime, oDTP.oData.dMinValue) === 3) oDTP.oData.dMinValue = new Date(dTempTime);
                    ...
                    if (oDTP.settings.parseDateTimeString) oDTP.oData.dCurrentDate = oDTP.settings.parseDateTimeString.call(oDTP, sCurrent, sMode, sFormat, $(oElement));
                    else oDTP.oData.dCurrentDate = oDTP._parseTime(sCurrent);
                } else if (oDTP.oData.bDateTimeMode) {
                    ...
                    if ($.cf._isValid(sMin)) oDTP.oData.dMinValue = oDTP._parseDateTime(sMin);
                    if ($.cf._isValid(sMax)) oDTP.oData.dMaxValue = oDTP._parseDateTime(sMax);
                    ...
                    if (sStartEnd !== '' && ($.cf._compare(sStartEnd, 'start') || $.cf._compare(sStartEnd, 'end')) && sStartEndElem !== '') {
                        if ($(sStartEndElem).length >= 1) {
                            ...
                            if (sTempDateTime !== '') {
                                ...
                                else dTempDateTime = oDTP._parseDateTime(sTempDateTime);

                                if ($.cf._compare(sStartEnd, 'start')) {
                                    if ($.cf._isValid(sMax)) {
                                        if (oDTP._compareDateTime(dTempDateTime, oDTP.oData.dMaxValue) < 0) oDTP.oData.dMaxValue = new Date(dTempDateTime);
                                    } else oDTP.oData.dMaxValue = new Date(dTempDateTime);
                                } else if ($.cf._compare(sStartEnd, 'end')) {
                                    if ($.cf._isValid(sMin)) {
                                        if (oDTP._compareDateTime(dTempDateTime, oDTP.oData.dMinValue) > 0) oDTP.oData.dMinValue = new Date(dTempDateTime);
                    ...
                    if (oDTP.settings.parseDateTimeString) oDTP.oData.dCurrentDate = oDTP.settings.parseDateTimeString.call(oDTP, sCurrent, sMode, sFormat, $(oElement));
                    else oDTP.oData.dCurrentDate = oDTP._parseDateTime(sCurrent);
                }
        ...
        _hidePicker: function (iDuration, oElementToShow) {
            ...
            if (oDTP.settings.beforeHide) oDTP.settings.beforeHide.call(oDTP, oElement);

            if (!$.cf._isValid(iDuration)) iDuration = oDTP.settings.animationDuration;
            ...
            if ($.cf._isValid(oElementToShow)) oDTP._showPicker(oElementToShow);
        },
        ...
        _modifyPicker: function () {
            ...
            for (iTempIndex = 0; iTempIndex < oDTP.settings.buttonsToDisplay.length; iTempIndex++) {
                if ($.cf._compare(oDTP.settings.buttonsToDisplay[iTempIndex], 'HeaderCloseButton')) bDisplayHeaderCloseButton = true;
                else if ($.cf._compare(oDTP.settings.buttonsToDisplay[iTempIndex], 'SetButton')) bDisplaySetButton = true;
                else if ($.cf._compare(oDTP.settings.buttonsToDisplay[iTempIndex], 'ClearButton')) bDisplayClearButton = true;
            }
            ...
            if (oDTP.settings.showHeader) {
                ...
                if (bDisplayHeaderCloseButton) sHeader += "<a class='dtpicker-close'>&times;</a>";
            ...
            for (iTempIndex = 0; iTempIndex < iNumberOfColumns; iTempIndex++) {
                ...
                if (oDTP.settings.readonlyInputs) sDTPickerComp += "<input type='text' class='dtpicker-compValue' readonly>";
                else sDTPickerComp += "<input type='text' class='dtpicker-compValue'>";
                sDTPickerComp += "<a class='dtpicker-compButton decrement'>" + oDTP.settings.decrementButtonContent + '</a>';
                if (oDTP.settings.labels) sDTPickerComp += "<div class='dtpicker-label'>" + oDTP.settings.labels[sFieldName] + '</div>';
                sDTPickerComp += '</div>';
                sDTPickerComp += '</div>';
            }
            ...
            if (bDisplaySetButton && bDisplayClearButton) sButtonContClass = ' dtpicker-twoButtons';
            else sButtonContClass = ' dtpicker-singleButton';

            var sDTPickerButtons = '';
            sDTPickerButtons += "<div class='dtpicker-buttonCont" + sButtonContClass + "'>";
            if (bDisplaySetButton) sDTPickerButtons += "<a class='dtpicker-button dtpicker-buttonSet'>" + oDTP.settings.setButtonContent + '</a>';
            if (bDisplayClearButton) sDTPickerButtons += "<a class='dtpicker-button dtpicker-buttonClear'>" + oDTP.settings.clearButtonContent + '</a>';
        ...
        _addEventHandlersForPicker: function () {
            ...
            if (!oDTP.settings.isInline) {
                $(document).on('click.DateTimePicker', function (e) {
                    if (oDTP.oData.bElemFocused) oDTP._hidePicker('');
                });
            }
            ...
            if (!oDTP.settings.readonlyInputs) {
                $('.dtpicker-compValue')
                    .not('.month .dtpicker-compValue, .meridiem .dtpicker-compValue')
                    .keyup(function () {
                ...
                $('.dtpicker-compValue').keyup(function (e) {
                    ...
                    if (
                        $oTextField.parent().hasClass('day') ||
                        $oTextField.parent().hasClass('hour') ||
                        $oTextField.parent().hasClass('minutes') ||
                        $oTextField.parent().hasClass('meridiem')
                    ) {
                        if (iLength > 2) {
                    ...
                    if (parseInt(e.keyCode ? e.keyCode : e.which) === 9) $(this).select();
                });
            }

            $(oDTP.element)
                .find('.dtpicker-compValue')
                .on('mousewheel DOMMouseScroll onmousewheel', function (e) {
            ...
            $(oDTP.element)
                .find('.dtpicker-close')
                .click(function (e) {
                    if (oDTP.settings.buttonClicked) oDTP.settings.buttonClicked.call(oDTP, 'CLOSE', oDTP.oData.oInputElement);
                    if (!oDTP.settings.isInline) oDTP._hidePicker('');
                });
            ...
            $(oDTP.element)
                ...
                .click(function (e) {
                    if (oDTP.settings.buttonClicked) oDTP.settings.buttonClicked.call(oDTP, 'SET', oDTP.oData.oInputElement);
                    oDTP._setButtonAction(false);
                });

            $(oDTP.element)
                .find('.dtpicker-buttonClear')
                .click(function (e) {
                    if (oDTP.settings.buttonClicked) oDTP.settings.buttonClicked.call(oDTP, 'CLEAR', oDTP.oData.oInputElement);
            ...
            if (oDTP.settings.captureTouchHold || oDTP.settings.captureMouseHold) {
                var sHoldEvents = '';
                if (oDTP.settings.captureTouchHold && oDTP.oData.bIsTouchDevice) sHoldEvents += 'touchstart touchmove touchend ';
                if (oDTP.settings.captureMouseHold) sHoldEvents += 'mousedown mouseup';
                ...
                $(oDTP.element)
                    .find('.day .increment, .day .increment *')
                    .click(function (e) {
                ...
                $(oDTP.element)
                    .find('.day .decrement, .day .decrement *')
                    .click(function (e) {
                        oDTP.oData.iCurrentDay--;
                        oDTP._setCurrentDate();
                        oDTP._setOutputOnIncrementOrDecrement();
                ...
                $(oDTP.element)
                    .find('.month .increment, .month .increment *')
                    .click(function (e) {
                ...
                $(oDTP.element)
                    .find('.month .decrement, .month .decrement *')
                    .click(function (e) {
                        oDTP.oData.iCurrentMonth--;
                        oDTP._setCurrentDate();
                        oDTP._setOutputOnIncrementOrDecrement();
                ...
                $(oDTP.element)
                    .find('.year .increment, .year .increment *')
                    .click(function (e) {
                ...
                $(oDTP.element)
                    .find('.year .decrement, .year .decrement *')
                    .click(function (e) {
                        oDTP.oData.iCurrentYear--;
                        oDTP._setCurrentDate();
                        oDTP._setOutputOnIncrementOrDecrement();
                ...
                $(oDTP.element)
                    .find('.hour .increment, .hour .increment *')
                    .click(function (e) {
                ...
                $(oDTP.element)
                    .find('.hour .decrement, .hour .decrement *')
                    .click(function (e) {
                        oDTP.oData.iCurrentHour--;
                        oDTP._setCurrentDate();
                        oDTP._setOutputOnIncrementOrDecrement();
                ...
                $(oDTP.element)
                    .find('.minutes .increment, .minutes .increment *')
                    .click(function (e) {
                ...
                $(oDTP.element)
                    .find('.minutes .decrement, .minutes .decrement *')
                    .click(function (e) {
                        oDTP.oData.iCurrentMinutes -= oDTP.settings.minuteInterval;
                        oDTP._setCurrentDate();
                        oDTP._setOutputOnIncrementOrDecrement();
                ...
                $(oDTP.element)
                    .find('.seconds .increment, .seconds .increment *')
                    .click(function (e) {
                ...
                $(oDTP.element)
                    .find('.seconds .decrement, .seconds .decrement *')
                    .click(function (e) {
                        oDTP.oData.iCurrentSeconds -= oDTP.settings.secondsInterval;
                        oDTP._setCurrentDate();
                        oDTP._setOutputOnIncrementOrDecrement();
            ...
            $(oDTP.element)
                .find('.meridiem .dtpicker-compButton, .meridiem .dtpicker-compButton *')
                .click(function (e) {
        ...
        _getValueOfElement: function (oElem) {
            ...
            if ($.cf._compare($(oElem).prop('tagName'), 'INPUT')) sElemValue = $(oElem).val();
            else sElemValue = $(oElem).html();
        ...
        _setValueOfElement: function (sElemValue, $oElem) {
            ...
            if (!$.cf._isValid($oElem)) $oElem = $(oDTP.oData.oInputElement);

            if ($.cf._compare($oElem.prop('tagName'), 'INPUT')) $oElem.val(sElemValue);
            else $oElem.html(sElemValue);

            var dElemValue = oDTP.getDateObjectForInputField($oElem);

            if (oDTP.settings.settingValueOfElement) oDTP.settings.settingValueOfElement.call(oDTP, sElemValue, dElemValue, $oElem);
        ...
        _bindTouchEvents: function (type) {
            ...
            $(oDTP.element)
                .find('.' + type + ' .increment, .' + type + ' .increment *')
            ...
            $(oDTP.element)
                .find('.' + type + ' .decrement, .' + type + ' .decrement *')
                .on('touchstart mousedown', function (e) {
                    e.stopPropagation();
                    if (!$.cf._isValid(oDTP.oData.sTouchButton)) {
                        oDTP.oData.iTouchStart = new Date().getTime();
        ...
        _parseDate: function (sDate) {
            ...
            if ($.cf._isValid(sDate)) {
                if (typeof sDate === 'string') {
                    var sArrDate;
                    if (oDTP.oData.bArrMatchFormat[4] || oDTP.oData.bArrMatchFormat[5] || oDTP.oData.bArrMatchFormat[6]) sArrDate = sDate.split(oDTP.settings.monthYearSeparator);
                    else sArrDate = sDate.split(oDTP.settings.dateSeparator);
        ...
        _parseTime: function (sTime) {
            ...
            var dTempDate = oDTP.settings.defaultDate ? new Date(oDTP.settings.defaultDate) : new Date(),
                ...
                sMeridiem,
                sArrTimeComp,
                bShowSeconds = oDTP.oData.bArrMatchFormat[0] || oDTP.oData.bArrMatchFormat[1];
            ...
            if ($.cf._isValid(sTime)) {
                if (typeof sTime === 'string') {
                    if (oDTP.oData.bIs12Hour) {
                        ...
                        if (!($.cf._compare(sMeridiem, 'AM') || $.cf._compare(sMeridiem, 'PM'))) sMeridiem = '';
                    }
                    ...
                    if (iHour === 12 && $.cf._compare(sMeridiem, 'AM')) iHour = 0;
                    else if (iHour < 12 && $.cf._compare(sMeridiem, 'PM')) iHour += 12;
                } else {
        ...
        _parseDateTime: function (sDateTime) {
            ...
            var dTempDate = oDTP.settings.defaultDate ? new Date(oDTP.settings.defaultDate) : new Date(),
                ...
                sArrTime,
                bShowSeconds =
                    oDTP.oData.bArrMatchFormat[0] || // "dd-MM-yyyy HH:mm:ss"
            ...
            if ($.cf._isValid(sDateTime)) {
                if (typeof sDateTime === 'string') {
                    ...
                    if ($.cf._isValid(sTime)) {
                        if (oDTP.oData.bIs12Hour) {
                            if ($.cf._compare(oDTP.settings.dateTimeSeparator, oDTP.settings.timeMeridiemSeparator) && sArrDateTime.length === 3) sMeridiem = sArrDateTime[2];
                            else {
        ...
        _getShortMonthIndex: function (sMonthName) {
            ...
            for (var iTempIndex = 0; iTempIndex < oDTP.settings.shortMonthNames.length; iTempIndex++) {
                if ($.cf._compare(sMonthName, oDTP.settings.shortMonthNames[iTempIndex])) return iTempIndex;
            }
        },
        ...
        _getFullMonthIndex: function (sMonthName) {
            ...
            for (var iTempIndex = 0; iTempIndex < oDTP.settings.fullMonthNames.length; iTempIndex++) {
                if ($.cf._compare(sMonthName, oDTP.settings.fullMonthNames[iTempIndex])) return iTempIndex;
            }
        },
        ...
        _setVariablesForDate: function (dInput, bIncludeTime, bSetMeridiem) {
            ...
            if (bValidInput) {
                ...
                if (!$.cf._isValid(bIncludeTime)) bIncludeTime = true;
                if (!$.cf._isValid(bSetMeridiem)) bSetMeridiem = true;
            } else {
                if (Object.prototype.toString.call(oDTP.oData.dCurrentDate) === '[object Date]' && isFinite(oDTP.oData.dCurrentDate)) dTemp = new Date(oDTP.oData.dCurrentDate);
                else dTemp = new Date();

                if (!$.cf._isValid(bIncludeTime)) bIncludeTime = oDTP.oData.bTimeMode || oDTP.oData.bDateTimeMode;
                if (!$.cf._isValid(bSetMeridiem)) bSetMeridiem = oDTP.oData.bIs12Hour;
            }
            ...
            if (bValidInput) return oDTV;
            else oDTP.oData = $.extend(oDTP.oData, oDTV);
        },
        ...
        _getValuesFromInputBoxes: function () {
            ...
            if (oDTP.oData.bDateMode || oDTP.oData.bDateTimeMode) {
                ...
                if (sMonth.length > 1) sMonth = sMonth.charAt(0).toUpperCase() + sMonth.slice(1);
            ...
            if (oDTP.oData.bTimeMode || oDTP.oData.bDateTimeMode) {
                ...
                if (oDTP.oData.bIs12Hour) {
                    if (oDTP.oData.iCurrentHour > 12) oDTP.oData.iCurrentHour = oDTP.oData.iCurrentHour % 12;
                } else {
                    if (oDTP.oData.iCurrentHour > 23) oDTP.oData.iCurrentHour = oDTP.oData.iCurrentHour % 23;
                }

                if (oDTP.oData.bIs12Hour) {
                    sMeridiem = $(oDTP.element).find('.meridiem .dtpicker-compValue').val();
                    if ($.cf._compare(sMeridiem, 'AM') || $.cf._compare(sMeridiem, 'PM')) oDTP.oData.sCurrentMeridiem = sMeridiem;

                    if ($.cf._compare(oDTP.oData.sCurrentMeridiem, 'PM')) {
                        if (oDTP.oData.iCurrentHour !== 12 && oDTP.oData.iCurrentHour < 13) oDTP.oData.iCurrentHour += 12;
                    }
                    if ($.cf._compare(oDTP.oData.sCurrentMeridiem, 'AM') && oDTP.oData.iCurrentHour === 12) oDTP.oData.iCurrentHour = 0;
                }
            }
        },
        ...
        _setCurrentDate: function () {
            ...
            var dTempDate = new Date(
                    ...
                    oDTP.oData.iCurrentDay,
                    oDTP.oData.iCurrentHour,
                    oDTP.oData.iCurrentMinutes,
                    oDTP.oData.iCurrentSeconds,
                    0,
                ),
                bGTMaxDate = false,
                bLTMinDate = false,
            ...
            if (oDTP.oData.dMaxValue !== null) bGTMaxDate = dTempDate.getTime() > oDTP.oData.dMaxValue.getTime();
            if (oDTP.oData.dMinValue !== null) bLTMinDate = dTempDate.getTime() < oDTP.oData.dMinValue.getTime();

            if (bGTMaxDate || bLTMinDate) {
                var bCDGTMaxDate = false,
                    bCDLTMinDate = false;
                if (oDTP.oData.dMaxValue !== null) bCDGTMaxDate = oDTP.oData.dCurrentDate.getTime() > oDTP.oData.dMaxValue.getTime();
                if (oDTP.oData.dMinValue !== null) bCDLTMinDate = oDTP.oData.dCurrentDate.getTime() < oDTP.oData.dMinValue.getTime();
            ...
            if (oDTP.oData.bDateMode || oDTP.oData.bDateTimeMode) {
                if (oDTP.oData.bDateMode && (oDTP.oData.bArrMatchFormat[4] || oDTP.oData.bArrMatchFormat[5] || oDTP.oData.bArrMatchFormat[6])) oDTP.oData.iCurrentDay = 1;
                ...
                if (oDTP.oData.bDateMode) {
                    if (oDTP.oData.bArrMatchFormat[4] || oDTP.oData.bArrMatchFormat[7])
                        // "MM-yyyy"
                    ...
                    else $(oDTP.element).find('.month .dtpicker-compValue').val(oFormattedDate.monthShort);
                } else $(oDTP.element).find('.month .dtpicker-compValue').val(oFormattedDate.monthShort);
                ...
                if (oDTP.settings.formatHumanDate) {
                    ...
                    if (
                        oDTP.oData.bDateMode &&
                        (oDTP.oData.bArrMatchFormat[4] || oDTP.oData.bArrMatchFormat[5] || oDTP.oData.bArrMatchFormat[6] || oDTP.oData.bArrMatchFormat[7])
                    ) {
                        if (oDTP.oData.bArrMatchFormat[4]) sDate = oFormattedDate.MM + oDTP.settings.monthYearSeparator + oFormattedDate.yyyy;
                        else if (oDTP.oData.bArrMatchFormat[5]) sDate = oFormattedDate.monthShort + oDTP.settings.monthYearSeparator + oFormattedDate.yyyy;
                        else if (oDTP.oData.bArrMatchFormat[6]) sDate = oFormattedDate.month + oDTP.settings.monthYearSeparator + oFormattedDate.yyyy;
                        else if (oDTP.oData.bArrMatchFormat[7]) sDate = oFormattedDate.yyyy + oDTP.settings.monthYearSeparator + oFormattedDate.MM;
            ...
            if (oDTP.oData.bTimeMode || oDTP.oData.bDateTimeMode) {
                ...
                if (oDTP.oData.bIs12Hour) $(oDTP.element).find('.meridiem .dtpicker-compValue').val(oDTP.oData.sCurrentMeridiem);
                ...
                if (oDTP.settings.formatHumanDate) {
                    ...
                    var bShowSecondsTime = oDTP.oData.bTimeMode && (oDTP.oData.bArrMatchFormat[0] || oDTP.oData.bArrMatchFormat[1]),
                        bShowSecondsDateTime =
                            oDTP.oData.bDateTimeMode &&
                            (oDTP.oData.bArrMatchFormat[0] ||
                                oDTP.oData.bArrMatchFormat[1] ||
                                oDTP.oData.bArrMatchFormat[2] ||
                                oDTP.oData.bArrMatchFormat[3] ||
                    ...
                    else sTime = oFormattedTime.hour + oDTP.settings.timeSeparator + oFormattedTime.mm;

                    if (oDTP.oData.bIs12Hour) sTime += oDTP.settings.timeMeridiemSeparator + oDTP.oData.sCurrentMeridiem;
                }
            }

            if (oDTP.settings.formatHumanDate) {
                if (oDTP.oData.bDateTimeMode) sFormat = oDTP.oData.sDateFormat;
                else if (oDTP.oData.bDateMode) sFormat = oDTP.oData.sTimeFormat;
                else if (oDTP.oData.bTimeMode) sFormat = oDTP.oData.sDateTimeFormat;

                sDateTime = oDTP.settings.formatHumanDate.call(oDTP, oDate, oDTP.settings.mode, sFormat);
            } else {
                if (oDTP.oData.bDateTimeMode) sDateTime = sDate + oDTP.settings.dateTimeSeparator + sTime;
                else if (oDTP.oData.bDateMode) sDateTime = sDate;
                else if (oDTP.oData.bTimeMode) sDateTime = sTime;
            }
        ...
        _formatTime: function (oDTVP) {
            ...
            if (iHour12 > 12) iHour12 -= 12;
            if (sHour === '00') iHour12 = 12;
            sHour12 = iHour12 < 10 ? '0' + iHour12 : iHour12;
            if (oDTP.oData.bIs12Hour) sHour = sHour12;
        ...
        _setButtons: function () {
            ...
            if (oDTP.oData.dMaxValue !== null) {
                if (oDTP.oData.bTimeMode) {
                    // Decrement Hour
                    if (
                        oDTP.oData.iCurrentHour + 1 > oDTP.oData.dMaxValue.getHours() ||
                        (oDTP.oData.iCurrentHour + 1 === oDTP.oData.dMaxValue.getHours() && oDTP.oData.iCurrentMinutes > oDTP.oData.dMaxValue.getMinutes())
                    )
            ...
            if (oDTP.oData.dMinValue !== null) {
                if (oDTP.oData.bTimeMode) {
                    // Decrement Hour
                    if (
                        oDTP.oData.iCurrentHour - 1 < oDTP.oData.dMinValue.getHours() ||
                        (oDTP.oData.iCurrentHour - 1 === oDTP.oData.dMinValue.getHours() && oDTP.oData.iCurrentMinutes < oDTP.oData.dMinValue.getMinutes())
                    )
            ...
            if (oDTP.oData.bIs12Hour) {
                ...
                if (oDTP.oData.dMaxValue !== null || oDTP.oData.dMinValue !== null) {
                    ...
                    if ($.cf._compare(oDTP.oData.sCurrentMeridiem, 'AM')) iTempHour += 12;
                    else if ($.cf._compare(oDTP.oData.sCurrentMeridiem, 'PM')) iTempHour -= 12;
        ...
        _compareTime: function (dTime1, dTime2) {
            var iTimeMatch = 0;
            if (dTime1.getHours() === dTime2.getHours() && dTime1.getMinutes() === dTime2.getMinutes()) iTimeMatch = 1; // 1 = Exact Match
            else {
                if (dTime1.getHours() < dTime2.getHours()) iTimeMatch = 2; // time1 < time2
                else if (dTime1.getHours() > dTime2.getHours()) iTimeMatch = 3; // time1 > time2
                else if (dTime1.getHours() === dTime2.getHours()) {
                    if (dTime1.getMinutes() < dTime2.getMinutes()) iTimeMatch = 2; // time1 < time2
                    else if (dTime1.getMinutes() > dTime2.getMinutes()) iTimeMatch = 3; // time1 > time2
                }
            }
            return iTimeMatch;
        },
